...
def calcVesselBoxCounts(graph, spacing, spacing_max):
  ...
  while spacing < spacing_max:
    ...
    for offset in offsets:
      ...
      volume_threshold = 1.
      #print(graph.nodes['position'], graph.edgelist, graph.edges['radius'], ld, volume_factor, volume_threshold)
      cnt += krebsutils.calc_vessel_boxcounts(graph.nodes['position'], graph.edgelist, graph.edges['radius'].flatten(), ld, volume_factor, volume_threshold)
